...
public class CheckoutServiceImpl extends BaseServiceImpl<Billing, Long> implements CheckoutService {
  ...
  /**
   * 通过计算，返回结算的简单账单 checkoutDate 办理出院日期，即截止计算日期
   */
  @Override
  public List<Billing> calculateCheckout(List<Billing> filteredBillings, Date checkoutDate) {
    ...
    for (Billing billing : filteredBillings) {
      ...
      if (bedNurseCharge != null && bedNurseCharge.getBedAmount() != null
          && bedNurseCharge.getNurseAmount() != null && bedNurseCharge.getPeriodStartDate() != null
          && bedNurseCharge.getPeriodEndDate() != null) {
        Long futureDays =
            DateTimeUtils.getDays(bedNurseCharge.getPeriodEndDate(), checkoutDate) + new Long(1);// 还差多少天
        ...
        if (bedNurseCharge.getChargeStatus().equals(PaymentStatus.PAID)) {
          if (pastDays > 0 && futureDays > 0) {// 已缴费，提前退床位护理，退款按天计算，不享受包月优惠价格
            bedRefundPayAmount =
                bedNurseCharge.getBedAmount().subtract(
                    bedAmountPerDay.multiply(new BigDecimal(pastDays)));// 已缴费（包月费） - 单价*已经住了的天数
            nurseRefundPayAmount =
                bedNurseCharge.getNurseAmount().subtract(
                    nurseAmountPerDay.multiply(new BigDecimal(pastDays)));// 已缴费（包月费） - 单价*已经住了的天数
            if (bedRefundPayAmount.signum() == -1) {
              bedRefundPayAmount = new BigDecimal(0);
              bedNurseRemark.append(noRefundDetail(bedNurseCharge.getPeriodStartDate(),
                  bedNurseCharge.getPeriodEndDate(), bedType, bedAmountPerMonth, bedAmountPerDay,
                  "bedCharge", bedNurseCharge.getBedAmount()));
            } else {
              bedRefundPayAmount = bedRefundPayAmount.negate();
              bedNurseRemark.append(refundDetail(bedNurseCharge.getPeriodStartDate(),
                  bedNurseCharge.getPeriodEndDate(), bedType, bedAmountPerMonth, bedAmountPerDay,
                  "bedCharge", bedNurseCharge.getBedAmount(), pastDays,
                  bedAmountPerDay.multiply(new BigDecimal(pastDays)), bedRefundPayAmount));
            }
            if (nurseRefundPayAmount.signum() == -1) {
              nurseRefundPayAmount = new BigDecimal(0);
              bedNurseRemark.append(noRefundDetail(bedNurseCharge.getPeriodStartDate(),
                  bedNurseCharge.getPeriodEndDate(), nurseLevel, nurseAmountPerMonth,
                  nurseAmountPerDay, "nurseCharge", bedNurseCharge.getBedAmount()));
            } else {
              nurseRefundPayAmount = nurseRefundPayAmount.negate();
              bedNurseRemark.append(refundDetail(bedNurseCharge.getPeriodStartDate(),
                  bedNurseCharge.getPeriodEndDate(), nurseLevel, nurseAmountPerMonth,
                  nurseAmountPerDay, "nurseCharge", bedNurseCharge.getBedAmount(), pastDays,
                  nurseAmountPerDay.multiply(new BigDecimal(pastDays)), nurseRefundPayAmount));
            }
          }
        }
        // 未缴费，未缴费(调账)或者已退款 ：少补 （正数）
          ...
          if (futureDays <= 0) {// 补缴当时的包月价格
            ...
            if ((bedRefundPayAmount.subtract(bedNurseCharge.getBedAmount())).signum() != -1) {
              bedRefundPayAmount = bedNurseCharge.getBedAmount();
              bedNurseRemark.append(fullPayDetail(bedNurseCharge.getPeriodStartDate(),
                  bedNurseCharge.getPeriodEndDate(), bedType, bedAmountPerMonth, bedAmountPerDay,
                  "bedCharge", bedNurseCharge.getBedAmount()));
            } else {
              bedNurseRemark.append(payDetail(bedNurseCharge.getPeriodStartDate(),
                  bedNurseCharge.getPeriodEndDate(), bedType, bedAmountPerMonth, bedAmountPerDay,
                  "bedCharge", bedNurseCharge.getBedAmount(), pastDays, bedRefundPayAmount));
            }
            if ((nurseRefundPayAmount.subtract(bedNurseCharge.getNurseAmount())).signum() != -1) {
              nurseRefundPayAmount = bedNurseCharge.getNurseAmount();
              bedNurseRemark.append(fullPayDetail(bedNurseCharge.getPeriodStartDate(),
                  bedNurseCharge.getPeriodEndDate(), nurseLevel, nurseAmountPerMonth,
                  nurseAmountPerDay, "nurseCharge", bedNurseCharge.getNurseAmount()));
            } else {
              bedNurseRemark.append(payDetail(bedNurseCharge.getPeriodStartDate(),
                  bedNurseCharge.getPeriodEndDate(), nurseLevel, nurseAmountPerMonth,
                  nurseAmountPerDay, "nurseCharge", bedNurseCharge.getNurseAmount(), pastDays,
                  nurseRefundPayAmount));
            }
          }
        }
      ...
      if (mealCharge != null && mealCharge.getMealAmount() != null
        ...
        Long futureDays =
            DateTimeUtils.getDays(mealCharge.getPeriodEndDate(), checkoutDate) + new Long(1);;// 还差多少天
        ...
        if (mealCharge.getChargeStatus().equals(PaymentStatus.PAID)) {
          if (pastDays > 0 && futureDays > 0) {// 已缴费，提前退伙食费，退款按天计算，不享受包月优惠价格
            mealRefundPayAmount =
                mealCharge.getMealAmount().subtract(
                    mealAmountPerDay.multiply(new BigDecimal(pastDays)));// 已缴费（包月费） - 单价*已经过了的天数
            if (mealRefundPayAmount.signum() == -1) {
              mealRefundPayAmount = new BigDecimal(0);
              mealRemark.append(noRefundDetail(mealCharge.getPeriodStartDate(),
                  mealCharge.getPeriodEndDate(), mealType, mealAmountPerMonth, mealAmountPerDay,
                  "mealCharge", mealCharge.getMealAmount()));
            } else {
              mealRefundPayAmount = mealRefundPayAmount.negate();
              mealRemark.append(refundDetail(mealCharge.getPeriodStartDate(),
                  mealCharge.getPeriodEndDate(), mealType, mealAmountPerMonth, mealAmountPerDay,
                  "mealCharge", mealCharge.getMealAmount(), pastDays,
                  mealAmountPerDay.multiply(new BigDecimal(pastDays)), mealRefundPayAmount));
            }
          }
        }
        // 未缴费，未缴费(调账)或者已退款 ：少补 （正数）
          ...
          if (futureDays <= 0) {// 补缴当时的包月价格
            ...
            if ((mealRefundPayAmount.subtract(mealCharge.getMealAmount())).signum() != -1) {
              mealRefundPayAmount = mealCharge.getMealAmount();
              mealRemark.append(fullPayDetail(mealCharge.getPeriodStartDate(),
                  mealCharge.getPeriodEndDate(), mealType, mealAmountPerMonth, mealAmountPerDay,
                  "mealCharge", mealCharge.getMealAmount()));
            } else {
              mealRemark.append(payDetail(mealCharge.getPeriodStartDate(),
                  mealCharge.getPeriodEndDate(), mealType, mealAmountPerMonth, mealAmountPerDay,
                  "mealCharge", mealCharge.getMealAmount(), pastDays, mealRefundPayAmount));
            }
          }
        }
      ...
      // waterElectricityCharge.getElectricityCount(),
      // waterElectricityCharge.getElectricityAmount()));
      // }
      //
      // }
      /**
        ...
        if (personalizedCharge.getChargeStatus().equals(PaymentStatus.UNPAID)
          ...
          for (PersonalizedRecord personalizedRecord : personalizedRecords) {
            ...
            personalizedRemark.append(personalizedDetail(personalizedRecord.getServiceTime(),
                personalizedRecord.getOperator(), personalizedNurse.getServicePrice(),
                personalizedRecord.getNurseContent()));
          }
        }
      }
    }
    ...
    if (filteredBillings != null && filteredBillings.size() > 0 && checkoutDate != null) {
      ...
      List<PersonalizedRecord> personalizedRecords =
          personalizedRecordService.searchRecentRecords(chargeSearch);
    ...
    if (StringUtils.isNotBlank(bedNurseRemark.toString().trim())) {
      bedNurseRemark.append(SpringUtils.getMessage("yly.checkout.refundPayNote",
    ...
    if (StringUtils.isNotBlank(mealRemark.toString().trim())) {
      mealRemark.append(SpringUtils.getMessage("yly.checkout.refundPayNote",
          formatDate(checkoutDate)));
    }
    ...
    totalAmount =
        bedAmount.add(depositAmount).add(nurseAmount).add(mealAmount).add(waterAmount)
            .add(electricityAmount).add(personalizedAmount);
  ...
  public void checkoutBillPayTransaction(Billing checkoutBill) throws Exception {
    ...
    for (Billing otherBilling : allOtherBillings) {
      if (otherBilling.getDeposit() != null) {
        ...
        otherBilling.getDeposit().setRemark(
            otherBilling
                .getDeposit()
                .getRemark()
                .replaceAll(SpringUtils.getMessage("yly.charge.unpaid.label"),
                    SpringUtils.getMessage("yly.charge.paid.label")));
      }
      ...
      if (otherBilling.getWaterElectricityCharge() != null) {
        ...
        // SpringUtils.getMessage("yly.charge.unpaid.label"),
        // SpringUtils.getMessage("yly.charge.paid.label")));
      }
      ...
      if (otherBilling.getPersonalizedCharge() != null) {
        ...
        otherBilling.getPersonalizedCharge().setRemark(
            otherBilling
                .getPersonalizedCharge()
                .getRemark()
                .replaceAll(SpringUtils.getMessage("yly.charge.unpaid.label"),
                    SpringUtils.getMessage("yly.charge.paid.label")));
        Set<PersonalizedRecord> personalizedRecords =
            otherBilling.getPersonalizedCharge().getPersonalizedRecords();
    ...
    Set<PersonalizedRecord> personalizedRecords =
        checkoutBill.getPersonalizedCharge().getPersonalizedRecords();
    if (personalizedRecords.size() > 0) {
      for (PersonalizedRecord personalizedRecord : personalizedRecords) {
        personalizedRecord.setChargeStatus(PaymentStatus.PAID);
      }
    ...
    if (checkoutBill.getBillingAdjustment() != null
        && checkoutBill.getBillingAdjustment().size() > 0) {
   ...
   */
  private String noRefundDetail(Date start, Date end, String service, BigDecimal aPerMonth,
      BigDecimal aPerDay, String chargeType, BigDecimal amount) {
    return SpringUtils.getMessage("yly.checkout.noRefundDetail", formatDate(start),
        formatDate(end), service, aPerMonth, aPerDay,
        SpringUtils.getMessage("yly.charge." + chargeType), amount);
  }

  /*
   * {0}-{1}：[{2} 月收费： ￥{3} 日收费： ￥{4}]\n 已缴{5}：￥ {6}，由于本次账单已用{7}天，按每天￥{4}的标准收取{5}，故实际消费了：￥
   * {8}，需退{5}：￥ {9}\n
   */
  private String refundDetail(Date start, Date end, String service, BigDecimal aPerMonth,
      BigDecimal aPerDay, String chargeType, BigDecimal amount, Long pastDays,
      BigDecimal caculAmount, BigDecimal finalAmount) {
    return SpringUtils.getMessage("yly.checkout.refundDetail", formatDate(start), formatDate(end),
        service, aPerMonth, aPerDay, SpringUtils.getMessage("yly.charge." + chargeType), amount,
        pastDays, caculAmount, finalAmount);
  }

  /*
   * {0}-{1}：[{2} 月收费： ￥{3} 日收费： ￥{4}]\n 未缴{5}：￥ {6},按天结算总金额达到包月价格，故需缴{5}：￥ {6}\n
   */
  private String fullPayDetail(Date start, Date end, String service, BigDecimal aPerMonth,
      BigDecimal aPerDay, String chargeType, BigDecimal amount) {
    return SpringUtils.getMessage("yly.checkout.fullPayDetail", formatDate(start), formatDate(end),
        service, aPerMonth, aPerDay, SpringUtils.getMessage("yly.charge." + chargeType), amount);
  }
   ...
   */
  private String payDetail(Date start, Date end, String service, BigDecimal aPerMonth,
      BigDecimal aPerDay, String chargeType, BigDecimal amount, Long pastDays,
      BigDecimal finalAmount) {
    return SpringUtils.getMessage("yly.checkout.payDetail", formatDate(start), formatDate(end),
        service, aPerMonth, aPerDay, SpringUtils.getMessage("yly.charge." + chargeType), amount,
        pastDays, finalAmount);
  }
   ...
   */
  // private String waterElecDetail(Date start, Date end, BigDecimal waterUnitPrice, BigDecimal
  // elecUnitPrice,
  ...
  // formatDate(start), formatDate(end), waterUnitPrice, elecUnitPrice, waterCount, waterAmout,
  // elecCount, elecAmount);
  // }
  /* 服务时间：{0} 服务护工：{1} 服务单价：{2} 服务内容：{3}\n */
  private String personalizedDetail(Date svcTime, String svcNurse, BigDecimal svcPrice,
      String nurseContent) {
    return SpringUtils.getMessage("yly.checkout.personalizedDetail", formatDate(svcTime), svcNurse,
        svcPrice, nurseContent);
  }
