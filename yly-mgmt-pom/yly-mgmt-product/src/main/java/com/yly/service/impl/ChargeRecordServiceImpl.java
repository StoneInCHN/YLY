...
public class ChargeRecordServiceImpl<T, ID extends Serializable> extends BaseServiceImpl<T, ID>
  ...
  // @Override
  // public Page<T> chargeRecordSearch(Boolean isTenant,Date beginDate, Date endDate, String
  // realName,
  ...
  public Page<T> chargeRecordSearch(ChargeSearchRequest queryParam, Pageable pageable) {
    ...
    try {
      ...
      if (queryParam.getStatus() != null) {
        ...
        Query filterQuery = new TermQuery(term);
        if (queryParam.getStatus().equals(PaymentStatus.UNPAID)) {
          BooleanQuery shouldQuery = new BooleanQuery();
          Term term_unpaid = new Term("chargeStatus", PaymentStatus.UNPAID.toString());
          Term term_unpaid_adjust = new Term("chargeStatus", PaymentStatus.UNPAID_ADJUSTMENT.toString());
          Query shouldFilter_unpaid = new TermQuery(term_unpaid);
          Query shouldFilter_unpaid_adjust = new TermQuery(term_unpaid_adjust);
          shouldQuery.add(shouldFilter_unpaid,Occur.SHOULD);
          shouldQuery.add(shouldFilter_unpaid_adjust,Occur.SHOULD);
      ...
      if (queryParam.getIsPeriod()) {
        if (queryParam.getBeginDate() != null) {
          TermRangeQuery tQuery =
              new TermRangeQuery("periodStartDate", DateTimeUtils.convertDateToString(
                  queryParam.getBeginDate(), null), null, true, false);
          query.add(tQuery, Occur.MUST);
        }
        if (queryParam.getEndDate() != null) {
          TermRangeQuery tQuery =
              new TermRangeQuery("periodEndDate", null, DateTimeUtils.convertDateToString(
                  queryParam.getEndDate(), null), false, false);
          query.add(tQuery, Occur.MUST);
        }
      } else {
        if (queryParam.getBeginDate() != null || queryParam.getEndDate() != null) {
          if (queryParam.getIsCreateTime()) {
            TermRangeQuery tQuery =
                new TermRangeQuery("createDate", DateTimeUtils.convertDateToString(
                    queryParam.getBeginDate(), null), DateTimeUtils.convertDateToString(
                    queryParam.getEndDate(), null), true, true);
            query.add(tQuery, Occur.MUST);
          } else {

            TermRangeQuery tQuery =
                new TermRangeQuery("payTime", DateTimeUtils.convertDateToString(
                    queryParam.getBeginDate(), null), DateTimeUtils.convertDateToString(
                    queryParam.getEndDate(), null), true, true);
            query.add(tQuery, Occur.MUST);
          }
