...
function populateFormLevel(form_sectionSize){//模块等级发生改变时候
			...
			if(editformLevelFlag){//当且仅当每个模块的等级都出来了，才能对整个评估表评等级
				...
				for(var i=0;  i<form_sectionSize; i++){
					var nextSectionLevel=$("#editsectionScore"+i).textbox('getValue');
				...
				$("#editsectionsResult").val(editsectionLevelList.join(";;;;"));	
				$.ajax({
					url:"../elderlyEvaluatingRecord/getCustomFormLevel.jhtml?formId="+$("#evaluatintFormID").val() +"&sectionLevels="+editsectionLevelList.join(";;;;"),
					type:"get",
					success:function(result,response,status){
