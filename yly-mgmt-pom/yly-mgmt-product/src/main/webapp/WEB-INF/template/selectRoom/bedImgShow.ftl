			      		 ...
			      		 [#list building.rooms as room]	
				   			<li>
				   				...
				   				[#list room.beds as bed]
					   				<div [#if bed.elderlyInfo]class="bed-item-somebody" data-elderlyId="${bed.elderlyInfo.id}" [#else] class="bed-item-nobody"  data-bedDescription="${building.buildingName} ${room.roomName} (${bed.bedNumber})"[/#if]  data-bedId="${bed.id}" data-bedNumber="${bed.bedNumber}">
